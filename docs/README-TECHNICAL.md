...
- 能把特典、OP/ED、PV 等非正片视频归入作品，但不影响正片观看进度。
...
- 本地目录授权和递归扫描。
- 视频文件过滤、附加视频识别、采样哈希、重复文件跳过、移动/缺失状态更新。
- 文件名解析和解析详情测试。
- Bangumi v0 搜索、详情和剧集接口调用。
- 导入审核页：候选选择、手动重新匹配、剧集映射、附加视频标签、批量偏移、重置解析、一键确认、一键取消。
- 首页：封面墙、搜索、年份/标签筛选、排序、最近、收藏、回收站、自定义合集。
- 放映厅：本地文件播放、正片/附加视频切换、文件源选择、播放进度、倍速、音量、全屏、字幕、评分、收藏、笔记。
- 笔记页：动画维度聚合、搜索、排序、Markdown 风格编辑/预览、内联截图图片、软删除、恢复、彻底删除。
...
- `files`：扫描到的视频文件，包含根目录、相对路径、父目录、大小、修改时间、采样哈希、扫描状态、媒体类型和附加视频标签。
- `match`：导入审核记录，包含目录 key、搜索词、候选 Bangumi ID、选择结果、草稿剧集映射、附加视频文件、警告和状态。
...
5. 如果目录名命中 OP/ED/NCOP/NCED/PV/特典/特别篇等规则，或父目录已有正片视频而子目录继续出现视频，则标记为 `media_kind: 'extra'`。
6. `computeQuickHash` 对文件头部、三分之一、三分之二和尾部采样，合并后计算 SHA-256。
7. 用 `size + quickHash` 判定文件身份，用路径辅助识别移动或更新。
8. 本轮未见到的旧文件标记为 `missing`，不直接删除。

难点与处理：

- 大文件全量哈希成本高：采用采样哈希，牺牲极低概率碰撞风险换取扫描速度。
- 文件移动不能只靠路径判断：优先用 `size + quickHash` 找回已有记录，再更新相对路径。
- 附加视频不能独立生成作品：扫描阶段记录 `extra_label` 和 `extra_group_path`，匹配阶段把它挂回父目录的 `MatchRecord.extra_file_ids`。
...
- 附加视频只写回 `anime_id`、`media_kind: 'extra'` 和标签，不写 `ep_id`。
...
- 附加视频显示在正片映射表下方，只展示文件名和可编辑标签，不参与集数映射。
- 对无法自动匹配的目录，提供手动关联入口。
...
观看进度按“已开始观看的最远正片集数”计算。附加视频播放不写入 `Episode` 进度，也不更新 `Anime.last_watched_episode`。首页提供手动修改看到第几集的入口，用于纠正误触或导入后的进度。

字幕支持 `.vtt`、`.srt`、`.ass` 和 `.ssa`。SRT/ASS/SSA 会在前端转换为 WebVTT 后加载；ASS 的样式、定位和特效不会保留。
...
- 放映厅可插入当前播放时间戳，也可截图后作为附件写入 IndexedDB，并以 `![截图](attachment:id)` 的 Markdown 形式插入笔记正文。
- 笔记区支持自动保存提示；截图在放映厅笔记区以缩略图预览，在全部笔记页面按正常内容宽度展示。
...
- 播放失败时保留错误提示。
- 如果浏览器解码出音频但没有可渲染视频帧，放映厅会提示当前格式需要转码或换成浏览器可播格式。
- 不在前端实现转码；需要转码时应进入完整版后端方案。

### 10.7 ASS 字幕样式限制

问题：原生 `<track>` 只能加载 WebVTT，不能直接渲染 ASS 的字体、描边、定位、卡拉 OK 和复杂特效。

处理：

- Lite 将 `.ass` / `.ssa` 解析为 WebVTT，让字幕文本先可用。
- 转换时剥离 ASS 样式标签，因此只保证文本和基础换行。
- 如果后续需要保留 ASS 样式，应引入专门的 ASS 渲染器；这不属于当前 Lite 的轻量实现范围。
...
2. 对一个自动匹配目录确认写库。
3. 对一个目录手动重新匹配并调整集数。
4. 导入包含特典或 OP/ED 子目录的作品，确认附加视频不会生成独立馆藏，也不会影响进度。
5. 首页搜索、筛选、收藏、加入合集、移入回收站和恢复，并验证观看进度修改。
6. 放映厅播放一个浏览器支持的视频，确认正片进度保存。
7. 在放映厅切换附加视频，确认可以播放但不写观看进度。
8. 加载 `.srt`、`.vtt` 或 `.ass` 字幕。
9. 创建带时间戳和截图的笔记，确认截图按 Markdown 位置渲染。
10. 导出 JSON，再在清空数据后导入。
